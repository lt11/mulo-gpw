...
dirOut <- file.path(dirBase,"gen", "os-par-vcf")
dir.create(path = dirOut, showWarnings = F, recursive = T)
inDir <- file.path(dirBase, "gen", "aln-par")
dirFastas <- file.path(dirBase, "rep")
...
stringInfo <- readLines(con = file.path(dirBase, "lib", "info-vcf.txt"),)
...
for (indF in allFiles) {
  ...
  nameFile <- basename(indF)
  idSample <- paste(unlist(strsplit(nameFile, split = "-"))[2],
                    collapse = "-")
  ...
  nameRef <- paste(unlist(strsplit(nameFile, split = "-"))[1],
                   collapse = "-")
  pathRef <- list.files(path = dirFastas,
